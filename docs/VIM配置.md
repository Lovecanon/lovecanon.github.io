<!-- 
Date: 2021-02-08
Title:
Tags: ["vim", "linux"]
Category: Linux
 -->
# VIM配置

## 升级vim
```bash
# 删除vim
yum list installed | grep vim
# 有的系统安装的是vim-minimal，删除时可能会报错，需要使用：rpm -e --nodeps vim-minimal
# 切不可按照它的提示删除，可能导致sudo也会被删除。sudo删除补救：pkexec yum install sudo
yum remove -y vim-common

# 安装依赖
yum install -y gcc make ncurses ncurses-devel git
yum install -y ctags tcl-devel python36-devel

# 安装vim。使用python3interp解决py3file报错
git clone https://github.com/vim/vim.git
cd vim
./configure --with-features=huge --enable-multibyte --enable-python3interp --enable-gui=no

make
make install

# 重开bash，查看vim版本
vim --version | head -1
```

## vim-clangd安装
```bash
# 安装依赖
yum -y install glibc
# yum直接安装cmake会报错，通过官网安装脚本安装。
wget https://github.com/Kitware/CMake/releases/download/v3.21.0/cmake-3.21.0-linux-x86_64.sh
chmod +x cmake-3.21.0-linux-x86_64.sh
./cmake-3.21.0-linux-x86_64.sh

# 安装llvm，llvm是一个编译工具集合
git clone --depth=1 https://github.com/llvm/llvm-project.git
cd llvm-project
mkdir build && cd build
# DLLVM_ENABLE_PROJECTS哪些项目需要编译
cmake -G "Unix Makefiles" -DLLVM_ENABLE_PROJECTS="clang;clang-tools-extra" -DCMAKE_INSTALL_PREFIX=~/tools/llvm -DCMAKE_BUILD_TYPE=Release ../llvm
make -j 16
make install
```

## Issue
### 解决`Error detected while processing /root/.vimrc:`警告
```bash
# 添加：export EDITOR=/usr/bin/vim
$ vim /root/.bashrc
```

### ultisnips运行错误
```bash
编译vim时需要：--enable-python3interp参数
```

## 参考：
* [clangd安装教程](https://jdhao.github.io/2021/07/03/install_clangd_on_linux/)
* [cmake官网](https://cmake.org/download/)
